...
plot_treemix = function(stem, o = NA, cex = 2, disp = 0.00002, plus = 0.0005, flip = vector(), arrow = 0.05, scale = F, ybar = -10.1, mbar = T, plotmig = T, plotnames = T, xmin = 0, lwd = 1, font = 1){
...
plot_resid = function(stem, pop_order, min = -0.009, max = 0.009, cex = 1, usemax = T, wcols = "r"){
	...
	names(c) = rownames(c)
	names(m) = rownames(m)
	o = pop_order  # read.table(pop_order, as.is = T, comment.char = "", quote = "")
...
get_tip_order <- function(tree){
	...
	tl = tree$tip.label[ordered_tips]
